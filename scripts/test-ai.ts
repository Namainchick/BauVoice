...
async function main() {
  ...
  // Answer with a short reply based on what the question asks
  ...
  const keywordFromQ = prevQuestionText.split(' ').find(w => w.length > 4) || '';
  if (keywordFromQ) {
    assertNoQuestionAbout(questions, keywordFromQ, '1b.2 Beantwortete Frage ist weg');
  } else {
    pass('1b.2 Beantwortete Frage ist weg (skipped — no keyword)');
  }
